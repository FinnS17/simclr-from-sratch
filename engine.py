...
def simclr_logits_targets(z1, z2, temp):
    ...
    # normalize = x / ||x|| -> scales vektor to length = 1
    # eg.:
    # normalize((1,2,3)) * (normalize((1,2,3)) = 1 -> same direction
    # normalize((1,2,3)) * (normalize((2,4,6)) = 1 -> same direction
    # normalize((1,0,1)) * (normalize((0,1,0)) = 0 -> othogonal
    # normalize((1,2,3)) * (normalize((-1,-2,-3)) = -1 -> opposite direction
    z1_norm, z2_norm = F.normalize(z1, dim=1), F.normalize(z2, dim=1)
    
    B = z1_norm.shape[0]
    Z = torch.cat([z1_norm, z2_norm], dim=0) # (2B x 128) -> eg. Z[1] <-> Z[b+1] (same origin, different corruption) -> positive pair
    S = Z @ Z.t() # (2B x 2B) -> all vs all similartiy (angle between embeddings of different pictures)

    # skale logits (with temperature -> adjusts sensitivity of softmax)
    logits = S / temp
    N = 2 * B
...
def train_one_epoch(encoder, projektor, loader, optimizer, device, temp):
    ...
    for x1, x2 in loader:
        ...
        loss = F.cross_entropy(logits, targets) # per row: softmax -> - log(p_pos) of targets[i]; avergage over 2b
        loss.backward()
        optimizer.step()
...
def eval_ssl_metrics(encoder, projektor, loader, device, temp, max_batches=40):
    ...
    total_hits = 0 # counts for psotive pair with highest probability
    total_ex = 0
